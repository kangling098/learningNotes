...
import { Form, Button, Row, Col, Input, Select, DatePicker, Cascader } from 'antd'
...
const Filter = ({
  ...
  const initialArea = []
  let { search_type =  defaultType, keyword, s_time, e_time, province, city, area } = filter
            ...
            hasArea ? (
              <FormItem {...formItemStyle}>
                {getFieldDecorator('areaData', {
                  ...
                  <Cascader options={citys} onChange={areaChange} changeOnSelect style={{width: '300px'}} placeholder="地区选择" />
                )}
              </FormItem>
            ) : (
              <FormItem {...formItemStyle}>
