...
import { query, remove, create, getLinkTypes } from '../services/index'
import { pageModel } from 'utils/model'
import { message } from 'antd'
const { prefix } = config

export default modelExtend(pageModel, {
  namespace: 'friendlinkLinks',
  state: {
    currentItem: {},
    modalVisible: false,
    modalType: 'create',
  ...
  subscriptions: {
    setup ({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === '/friendlink/links') {
  ...
  effects: {
    ...
    * delete ({ payload }, { call, put, select }) {
      ...
      const { selectedRowKeys } = yield select(_ => _.friendlinkLinks)
      if (data.success) {
