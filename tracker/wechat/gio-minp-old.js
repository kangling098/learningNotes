...
class Page$1 {
  ...
  touch(t) { // t应该为页面 page对象, touch方法更新currentPage的path 与 query.
    this.path = t.route, this.time = Date.now(), this.query = this.queries[t.route] ? this.queries[t.route] : void 0
  }
  addQuery(t, e) { // t为页面page对象, e为query对象,传递过来了query对象时,将query对象序列化为字符串,赋值给对象this.quries[t.route] = query序列化字符串
...
class Observer {
  ...
  pageListener(t, e, i) {
    if (this.growingio.debug && console.log("Page.", t.route, "#", e, Date.now()), "onShow" === e) this.isPauseSession ? this.isPauseSession = !1 : (this.currentPage.touch(t), this.sendPage(t));
      ...
      var s = null,
        n = null;
      2 > i.length ? 1 === i.length && (i[0].from ? s = i[0] : i[0].title && (n = i[0])) : (s = i[0], n = i[1]), this.pauseSession(), this.sendPageShare(t, s, n)
    ...
    if (t && t.length > 0) {
      ...
      if (e && e.length > 1) {
        ...
        if (t && t.length > 1) {
          ...
          i && i.length > 2 && (s.page = i[1], s.function = i[2]), this._sendEvent({
            t: "cstm",
            ptm: this.currentPage.time,
            p: this.currentPage.path,
            q: this.currentPage.query,
  ...
  sendClick(t, e) {
    ...
    s.dataset.title ? n.v = s.dataset.title : s.dataset.src && (n.h = s.dataset.src), void 0 !== s.dataset.index && (n.idx = /^[\d]+$/.test(s.dataset.index) ? parseInt(s.dataset.index) : -1), i.e = [n], this._sendEvent(i)
  }
  sendSubmit(t, e) {
    var i = {
      t: "sbmt",
    ...
    return function () {
      ...
      if (this._growing_page_ && -1 !== VdsInstrumentAgent.pageHandlers.indexOf(t)) {
        ...
        if (-1 !== ["onShow", "onLoad", "onTabItemTap", "onHide"].indexOf(t)) i = e.apply(this, arguments);
          ...
          r && r.followShare && i && i.path && (i.path = -1 === i.path.indexOf("?") ? i.path + "?suid=" + r.weixin.uid : i.path + "&suid=" + r.weixin.uid)
        }
      }
      return i
    }
...
class GrowingIO {
  ...
  _proxyTaro(t) {
    ...
    t.createComponent = function (t, s) {
      let n = t;
      for (; n && n.prototype;) {
        const i = Object.keys(Object.getOwnPropertyDescriptors(n.prototype) || {});
        for (let s = 0; i.length > s; s++)
          if (i[s].startsWith("func__")) {
            const r = n.name,
              o = i[s].slice(6);
            e.taroRootVMs[i[s]] = r + "_" + ("" + t.prototype[i[s]]).match(/this\.__triggerPropsFn\(\"(.+)\",/)[1] + "_" + o
          } n = Object.getPrototypeOf(n)
      }
      const r = i(t, s);
      return s && VdsInstrumentAgent.instrumentTaroPageComponent(r), r
    }
  }
  _proxyVue(t) {
    if (void 0 !== t.mixin) {
