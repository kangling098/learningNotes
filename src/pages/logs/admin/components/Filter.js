...
import { Form, Button, Icon, Row, Col, DatePicker, Input, Cascader, Switch, Select } from 'antd'
...
const Filter = ({ 
  ...
  filter,
  form: { // form来源自 Filter导出之前被 Form.create()(Filter)增强之后,给Filter增加了form属性
  ...
  const handleReset = () => { // 对重置按钮点击的处理函数
  ...
  const handleChange = (key, values) => { // 用来处理一些表单的数据改变,当数据改变时,手动更改提交数据,最后调用查询借口(onFilterChange)
  ...
  if (create_time && create_time[0]) { // 当通过filter传入了初始时间时,通过moment将初始时间转换
  ...
  return (
    <Row gutter={10} style={{marginBottom:'15px'}} type="flex" justify="start" >
      ...
      {otherSelection&&otherSelection.map((val,key)=>(
        <Col key={key}>
